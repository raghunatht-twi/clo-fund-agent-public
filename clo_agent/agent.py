"""CLO fund-performance agent — Claude Sonnet 4.6 with tool use over the workbook."""
...
Your data source is the fund workbook containing all 8 data products (DP-01 through
DP-08) for DKIG-2024-VII and DKIG-2016-I.
...
FUND IDENTIFICATION RULES
──────────────────────────
• Available funds: DKIG-2024-VII and DKIG-2016-I.
...
• If the user does not specify a fund and the intent is not clearly comparative:
    - Ask the user which fund they mean.
...
def _build_system_prompt() -> list[dict]:
    ...
    return [
        {"type": "text", "text": ROLE_INSTRUCTIONS},
        {
            ...
            "text": (
                "===== ONTOLOGY (JSON-LD) =====\n"
                "Use this to interpret terminology, traverse relationships, "
                "and pick the right data product for each question.\n\n"
                + ontology
            ),
...
# ---------------------------------------------------------------------------
USAGE = """\
CLO Fund Performance Agent (workbook backend)

Usage:
  python -m clo_agent 'What is the fund net IRR?'   # one-shot
  python -m clo_agent                                 # interactive REPL
...
Environment variables:
  ...
  CLO_WORKBOOK_SHA256       Expected SHA-256 of the workbook file (optional)
"""
...
def _run_repl() -> int:
    ...
    session_tokens = 0
    print("\033[1mCLO Fund Performance Agent\033[0m  (workbook backend)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(argv) > 1:
        print(
            ...
            "  python -m clo_agent 'your question here'",
            file=sys.stderr,
        )
        return 2
    if argv:
